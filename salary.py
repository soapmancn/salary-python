...
count = len(driver.find_elements(By.TAG_NAME, 'tr'))
...
for num in range(7, count):
    td_list = []
    tdContent = driver.find_elements(By.TAG_NAME, 'tr').__getitem__(num).find_elements(By.TAG_NAME, 'td')
    for i in tdContent:
        td_list.append(i.text)
    td_arrays.append(td_list)
driver.quit()

# 组装消息
for td_list in td_arrays:
    message = ''
    fgx = '-----------------------'
    hhf = '\n'
    year = ''
